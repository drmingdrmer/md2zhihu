...
def _process_table(header, align):
    ...
    for i, v in enumerate(aligns):
        ...
        if len(headers) > i:
            cells.append({"type": "table_cell", "text": headers[i], "params": (aligns[i], True)})

    i += 1
    while i + 1 < len(headers):
        cells.append({"type": "table_cell", "text": headers[i], "params": (None, True)})
        aligns.append(None)
        i += 1
...
def _process_row(row, aligns):
    ...
    for i, s in enumerate(re.split(r" *(?<!\\)\| *", row)):
        ...
        if len(aligns) < i + 1:
            cells.append({"type": "table_cell", "text": text, "params": (None, False)})
        else:
            cells.append({"type": "table_cell", "text": text, "params": (aligns[i], False)})
